...
class TurnManager {
    ...
    nextTurn() {
        ...
        if (nextPlayer.status.skippedTurns > 0) {
            ...
            // Отправляем специальное событие для модального окна пропуска
            this.gameState.io.emit('game:notification', {
                title: 'ПРОПУСК ХОДА',
                message: `Вы пропускаете этот ход. Осталось пропусков: ${nextPlayer.status.skippedTurns}`,
                type: 'info',
                playerName: nextPlayer.displayName,
                action: 'interactive_skip' // Кнопка OK вызовет player:finish_turn
            });
